...
function setupToolbarTabs(root) {
  const tabButtons = Array.from(root.querySelectorAll("[data-toolbar-tab]"));
  const tabPanels = Array.from(root.querySelectorAll("[data-toolbar-panel]"));
  ...
  const validPanelIds = new Set(
    tabPanels
      .map((panel) => panel.dataset.toolbarPanel)
      .filter((panelId) => typeof panelId === "string" && panelId.length > 0)
  );

  const firstTabId = tabButtons[0]?.dataset.toolbarTab;
  let activeTabId = tabButtons.find((button) => button.getAttribute("aria-selected") === "true")?.dataset.toolbarTab;
  if (!activeTabId || !validPanelIds.has(activeTabId)) {
    activeTabId = firstTabId;
  }

  function applyActiveTab(nextTabId) {
    if (!nextTabId || !validPanelIds.has(nextTabId)) {
      return;
    }
    activeTabId = nextTabId;
    for (const button of tabButtons) {
      const isActive = button.dataset.toolbarTab === nextTabId;
      button.setAttribute("aria-selected", isActive ? "true" : "false");
      button.tabIndex = isActive ? 0 : -1;
    }
    for (const panel of tabPanels) {
      const isActive = panel.dataset.toolbarPanel === nextTabId;
  ...
  for (const button of tabButtons) {
    button.addEventListener("click", () => {
      applyActiveTab(button.dataset.toolbarTab);
    });
  }

  applyActiveTab(activeTabId);
}
    ...
    <section class="panel editor-frame" aria-label="Editor">
      <div class="toolbar" role="toolbar" aria-label="Editor toolbar">
        <div class="toolbar-tabs" role="tablist" aria-label="Toolbar sections">
          <button type="button" class="toolbar-tab" role="tab" data-toolbar-tab="drawing" aria-selected="true">Drawing</button>
          <button type="button" class="toolbar-tab" role="tab" data-toolbar-tab="rooms" aria-selected="false">Rooms</button>
          <button type="button" class="toolbar-tab" role="tab" data-toolbar-tab="openings" aria-selected="false">Openings</button>
          <button type="button" class="toolbar-tab" role="tab" data-toolbar-tab="lighting" aria-selected="false">Lighting</button>
          <button type="button" class="toolbar-tab" role="tab" data-toolbar-tab="estimate" aria-selected="false">Estimate</button>
          <button type="button" class="toolbar-tab" role="tab" data-toolbar-tab="plan" aria-selected="false">Plan</button>
        </div>
        <div class="toolbar-panels">
          <section class="toolbar-tab-panel" role="tabpanel" data-toolbar-panel="drawing">
          ...
          <section class="toolbar-tab-panel" role="tabpanel" data-toolbar-panel="rooms" hidden>
          ...
          <section class="toolbar-tab-panel" role="tabpanel" data-toolbar-panel="openings" hidden>
          ...
          <section class="toolbar-tab-panel" role="tabpanel" data-toolbar-panel="lighting" hidden>
          ...
          <section class="toolbar-tab-panel" role="tabpanel" data-toolbar-panel="estimate" hidden>
          ...
          <section class="toolbar-tab-panel" role="tabpanel" data-toolbar-panel="plan" hidden>
